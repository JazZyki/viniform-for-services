...
export const calculateDentPrice = (
    ...
    if (useGlobalPrices) {
        ...
        price3 = getGlobalBasePrice(diameter3);
        price4 = getGlobalBasePrice(diameter4);
        price5 = getGlobalBasePrice(diameter5);
    } else {
        price1 = DENT_PRICELIST[diameter1] || 0;
        price2 = DENT_PRICELIST[diameter2] || 0;
        price3 = DENT_PRICELIST[diameter3] || 0;
        price4 = DENT_PRICELIST[diameter4] || 0;
        price5 = DENT_PRICELIST[diameter5] || 0;
    }

    const c1 = parseInt(count1) || 0;
    const c2 = parseInt(count2) || 0;
    const c3 = parseInt(count3) || 0;
    const c4 = parseInt(count4) || 0;
    const c5 = parseInt(count5) || 0;

    if ((price1 === 0 || c1 === 0) && (price2 === 0 || c2 === 0) && (price3 === 0 || c3 === 0) && (price4 === 0 || c4 === 0) && (price5 === 0 || c5 === 0)) return 0;

    let allDentsOnPart = [];
    for (let i = 0; i < c1; i++) allDentsOnPart.push(price1);
    for (let i = 0; i < c2; i++) allDentsOnPart.push(price2);
    for (let i = 0; i < c3; i++) allDentsOnPart.push(price3);
    for (let i = 0; i < c4; i++) allDentsOnPart.push(price4);
    for (let i = 0; i < c5; i++) allDentsOnPart.push(price5);
